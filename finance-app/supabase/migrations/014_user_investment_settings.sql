...
-- значення жили тільки в localStorage (free-cash.ts/stock-manual-invested.ts)
-- або виводились із purchase_price активів (scaleInvestedByType для крипти) —
-- пристрій-специфічно чи неявно, звідси скарги, що поле "збивається". Тепер —
-- реальна таблиця з RLS, синхронізується між пристроями так само, як інші дані.
-- ============================================================
...
-- Якщо міграцію 014 вже застосовували РАНІШЕ (до появи crypto-колонки) —
-- CREATE TABLE IF NOT EXISTS вище нічого не змінить, тому колонку додаємо
-- окремо й ідемпотентно.
ALTER TABLE public.user_investment_settings
  ADD COLUMN IF NOT EXISTS crypto_manual_invested_usd_minor BIGINT NOT NULL DEFAULT 0;
