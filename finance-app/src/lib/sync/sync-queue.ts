...
// LocalTransaction / LocalCategory / LocalAccount — використовуються у деструктуризації нижче
void (0 as unknown as LocalTransaction | LocalCategory | LocalAccount)
...
export async function hasPendingRecords(userId: string): Promise<boolean> {
  const [txCount, catCount, accCount] = await Promise.all([
...
export async function flushSyncQueue(userId: string): Promise<PushResult> {
  ...
  return {
    successCount: txResult.successCount + catResult.successCount + accResult.successCount,
    errorCount: txResult.errorCount + catResult.errorCount + accResult.errorCount,
    errors: [...txResult.errors, ...catResult.errors, ...accResult.errors],
  }
}
